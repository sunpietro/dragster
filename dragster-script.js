...
var Dragster = function(params) {
  ...
  findDraggableElements = function() {
    return [].slice.call(
      document.querySelectorAll(finalParams.elementSelector)
    );
  };
  ...
  cleanWorkspace = function(element, eventName) {
    ...
    [].slice
      .call(document.getElementsByClassName(CLASS_DRAGGABLE))
      .forEach(function(dragEl) {
  ...
  cleanReplacables = function() {
    [].slice
      .call(document.getElementsByClassName(CLASS_REPLACABLE))
  ...
  updateRegionsHeight = function() {
    if (finalParams.updateRegionsHeight) {
      var regions = [].slice.call(
        document.getElementsByClassName(CLASS_REGION)
      );

      regions.forEach(function(region) {
        var elements = [].slice.call(
            region.querySelectorAll(finalParams.elementSelector)
          ),
  ...
  regionEventHandlers = {
    ...
    mousedown: function(event) {
      ...
      if (
        finalParams.onBeforeDragStart(event) === FALSE ||
    ...
    mousemove: function(event) {
      ...
      if (
        finalParams.onBeforeDragMove(event) === FALSE ||
        !shadowElementRegion
      ) {
        return FALSE;
      }
      ...
      var eventObject = event.changedTouches ? event.changedTouches[0] : event,
        ...
        isTargetRegionDragOnly =
          unknownTarget.classList.contains(
            finalParams.dragOnlyRegionCssClass
          ) && isAllowedTarget,
    ...
    mouseup: function(event) {
      ...
      findByClass = finalParams.replaceElements
        ? CLASS_REPLACABLE
        : CLASS_PLACEHOLDER;
  ...
  dropActions = {
    ...
    moveElement: function(dragsterEvent, dropTarget, dropDraggableTarget) {
      ...
      if (
        draggedElement.firstChild &&
        finalParams.wrapDraggableElements === TRUE
      ) {
